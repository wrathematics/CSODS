...
title: "Assignment 2"
subtitle: "DSE 512"
date: "Assigned 2022-2-15 --- Due 2022-2-22 11:59pm"
output: 
    - pdf_document
urlcolor: blue
linkcolor: blue
...
Your goal will be to create a Singularity container satisfying some basic requirements that you upload to a shared space on ISAAC and use to run a small job on the cluster. You can build the container wherever you like (I recommend an EC2 instance that you terminate afterwards). And you can use whatever you like for software management within the container (e.g. `apt-get`, pip, conda, install from source, whatever), *so long as* all other requirements are satisfied and no extra steps are required to run software in your container.

1. Create a Dockerfile that uses `ubuntu:20.04` as the base image and installs:
    * R (4.x)
...
2. Build the Docker image. Call it `dse512`. List the commands you run to build this image.
3. Convert the Docker image to a Singularity image. Name this image `dse512_${NETID}.simg`. So for example, mine would be `dse512_mschmid3.simg`. List the commands you run to build this image.
4. Create a directory in the course project space in lustre on ISAAC (project number UTK0188). The directory should be named your UT netid. Upload your singularity image to this directory. List the command(s) you run to transfer the file.
5. Create a job script in your project directory that will run an R "hello world" (`print('hello world')` will do) using your singularity image. If you can not use the project account UTK0188, then use the opportunistic account UTK0011.
6. Run the job and provide the output.
