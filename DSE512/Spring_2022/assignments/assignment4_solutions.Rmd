...
3. (15%) Give the code to fit all of these models in parallel using fork. For simplicity, you may assume the number of cores `p` is fixed at 8 throughout. (Hint: I recommend the rows-as-inputs strategy mentioned in class using R's `expand.grid()` or similar).
    ...
    ```r
    get_param_grid = function(ncols)
    {
      column_option = list(c(TRUE, FALSE))
      expand.grid(rep(column_option, ncols))
    }
    
    get_model_data = function(x, columns)
    {
      x[, columns, drop=FALSE]
    }
    
    fit_single_model = function(i, grid, x, y)
    {
      columns = grid[i, ] |> unlist() |> unname()
      x_mdl = get_model_data(x, columns)
      fit_model(x_mdl, y)
    }
    
    fit_all_models = function(x, y, ncores=8)
    {
      n = ncol(x)
      grid = get_param_grid(n)
      grid = grid[-nrow(grid), ]
      parallel::mclapply(1:nrow(grid), FUN=fit_single_model, grid=grid, x=x, y=y)
    }
    
    pick_best_model = function(models)
    {
      aic_scores = sapply(models, AIC)
      idx = which.min(aic_scores)
      list(aic=aic_scores[idx], model=models[[idx]])
    }
    
    ### Example
    fit_model = function(x, y) lm(y~x)
    data(mtcars)
    y = mtcars[, 1]
    x = as.matrix(mtcars[, -1])
    models = fit_all_models(x, y)
    best_aic = pick_best_model(models)
    best_aic
    ```
...
1. (15%) Poisson Regression is a generalized linear model (GLM) used to model dependent variables which are counts. It was originally used to model the number of expected mule kicks to the head for Prussian soldiers. Its cost function is:
    ...
    {
      eta = X %*% theta
      J_local = -sum(y*eta - linkinv_poisson(eta))
      pbdMPI::allreduce(J_local)
    }
    
    poisson_fit = function(X, y, maxiter=100)
    {
    ...
    my_rows = get_my_tasks(nrow(X), comm.size(), comm.rank())
    X_local = X[my_rows, ]
    y_local = y[my_rows]
    
    ret = poisson_fit(X_local, y_local)
    comm.print(ret$par)
    
    if (comm.rank() == 0){
      glm(formula=breaks~wool+tension, data=warpbreaks, family=poisson)$coefficients
    }
